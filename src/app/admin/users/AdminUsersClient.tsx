...
import { Users, ShieldCheck, ShieldOff, Trash2, Search, RefreshCw } from 'lucide-react'
...
export default function AdminUsersClient({ currentUserId }: { currentUserId: string }) {
  ...
  const [tab, setTab] = useState<'all' | 'admin' | 'users'>('all')
  ...
  const filtered = users.filter((u) => {
    const matchTab = tab === 'all' || (tab === 'admin' ? u.is_admin : !u.is_admin)
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={load} className="flex items-center gap-1.5 text-sm text-brand-gray hover:text-primary transition-colors">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
          {(['all', 'admin', 'users'] as const).map((t) => (
