...
export async function GET() {
  ...
  await Promise.all([ensureNeopulseTables(), ensureWellnessTables()])
  ...
  const users = await query<{
    ...
    `SELECT
       ...
       (SELECT MAX(check_in_date)::text FROM wellness_checkins wc WHERE wc.user_id = u.id) AS last_checkin,
       (SELECT ROUND(AVG(wellness_score::numeric),1)::text FROM wellness_checkins wc
        WHERE wc.user_id = u.id AND check_in_date >= CURRENT_DATE - 7) AS avg_wellness,
       (SELECT COUNT(*)::text FROM wellness_checkins wc WHERE wc.user_id = u.id) AS checkin_streak
     FROM users u
     LEFT JOIN neopulse_transactions t ON t.user_id = u.id
     WHERE u.neopulse_balance > 0 OR EXISTS (SELECT 1 FROM wellness_checkins wc WHERE wc.user_id = u.id)
     GROUP BY u.id, u.name, u.email, u.neopulse_balance, u.referral_code
     ORDER BY u.neopulse_balance DESC`,
    []
  )

  const stats = await query<{ total_balance: string; total_transactions: string; active_today: string }>(
    `SELECT
  ...
  return NextResponse.json({ users, stats: stats[0] ?? { total_balance: '0', total_transactions: '0', active_today: '0' } })
}
