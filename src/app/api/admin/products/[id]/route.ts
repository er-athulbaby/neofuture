...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    custom_gst_rate, pack_format, serving_size, min_order_qty, variant_label,
  } = await req.json()
  ...
  await query(
    ...
    [name, slug, category_id ?? null, price, sale_price ?? null, stock ?? 0, sku ?? null,
     ...
     custom_gst_rate ?? null, pack_format ?? null, serving_size ?? null, min_order_qty ?? 1,
     variant_label ?? null, id]
  )
