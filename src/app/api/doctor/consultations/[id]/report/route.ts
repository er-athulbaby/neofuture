...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pdfUrl = await uploadToS3(pdfBuffer, s3Key, 'application/pdf')
  await query('UPDATE consultation_reports SET pdf_url=$1 WHERE id=$2', [pdfUrl, existing.id])

  if (patient?.email) {
    await sendConsultationReport(patient.email, { patientName: patient.name, doctorName: doctor.name, pdfBuffer }).catch(() => {})
  }
