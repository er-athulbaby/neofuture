...
export async function GET(_req: NextRequest, { params }: Props) {
  ...
  const hasAccess = await queryOne(
    `SELECT 1 FROM consultations WHERE doctor_id=$1 AND patient_id=$2 AND status IN ('confirmed','completed') LIMIT 1`,
    [doctor.id, patientId]
  )
  ...
  const [patient, vitals, consultations] = await Promise.all([
    queryOne<{ id: string; name: string; email: string; phone: string | null }>(
      'SELECT id, name, email, phone FROM users WHERE id = $1', [patientId]
    ),
    queryOne<{
    ...
    query(`
      ...
      WHERE c.doctor_id=$1 AND c.patient_id=$2 AND c.status IN ('confirmed','completed')
